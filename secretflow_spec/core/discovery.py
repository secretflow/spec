...
def _check_module_usage(file_path: str) -> bool:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            has_parse_import = True
            # Do not compare module names because of indirect import
            if any(item.name == "Component" for item in node.names):
                return True
    ...
    for dir_name in root_dirs:
        ...
        for pyfile in glob.glob(pattern, recursive=True):
            ...
            try:
                ...
                if verbose:
                    logging.warning(f"import component {module_name} from {pyfile}")
