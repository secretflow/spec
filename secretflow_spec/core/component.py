...
try:
    ...
    # Define a no-op decorator if dataclass_transform is not available
...
@dataclass
class Output(BuiltinType):
    uri: str
    data: DistData = None


@dataclass
class UnionSelection(BuiltinType):
    name: str
    desc: str
    minor_min: int = 0
    minor_max: int = -1
    ...
    def __init_subclass__(cls, **kwargs):
        def _is_custom_class(cls):
            if not isinstance(cls, type) or cls.__module__ == "builtins":
                return False
            if issubclass(cls, (DistData, BuiltinType)):
                return False
            return True
