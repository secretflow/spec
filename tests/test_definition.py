...
def test_custom():
    from tests.spec.extend.calculate_rules_pb2 import CalculateOpRules

    class CustomComponent(Component):
        rule: CalculateOpRules = Field.custom_attr(desc="xx")

        def evaluate(self): ...

    d = Definition(CustomComponent, "test", "0.0.1")
    protobuf_cls = d.component_def.attrs[0].custom_protobuf_cls
    assert protobuf_cls == "tests.spec.extend.calculate_rules_pb2.CalculateOpRules"
...
def test_version():
    class VersionComponent(Component):
        ...
        input1: Input = Field.input(types=[DistDataType.NULL], minor_min=0, minor_max=0)
        input2: Input = Field.input(types=[DistDataType.NULL], minor_min=1)

        output1: Output = Field.output(
            types=[DistDataType.NULL], minor_min=0, minor_max=0
        )
        output2: Output = Field.output(types=[DistDataType.NULL], minor_min=1)

        def evaluate(self) -> None: ...

    definition = Definition(VersionComponent, "test", "0.2.0")
    comp_def = definition.component_def
    assert len(comp_def.attrs) == 3 and comp_def.attrs[2].name == "v2"
    assert len(comp_def.inputs) == 2 and len(comp_def.outputs) == 2

    input = Input(type=str(DistDataType.NULL))
    args = {"v0": 0, "input1": input, "output1": Output(uri="o1")}
    args["_minor"] = 0
    comp: VersionComponent = definition.make_component(args)
    assert comp.v0 == 0 and not comp.v1 and not comp.v2

    args = {"v0": 0, "v1": 1, "input2": input, "output2": Output(uri="o1")}
    args["_minor"] = 1
    comp: VersionComponent = definition.make_component(args)
    assert comp.v0 == 0 and comp.v1 == 1 and not comp.v2
...
def test_outdated_vertical_table():
    class TestOutdatedVerticalTable(Component):
        input1: Input = Field.input("", types=[DistDataType.OUTDATED_VERTICAL_TABLE])

        def evaluate(self) -> None: ...

    with pytest.raises(Exception) as e:
        Definition(TestOutdatedVerticalTable, "test", "0.0.1")
    logging.info(f"Caught expected Exception: {e}")

    class TestOutdatedVerticalTable1(Component):
        input1: Input = Field.input("", types=[DistDataType.VERTICAL_TABLE])

        def evaluate(self) -> None: ...

    d1 = Definition(TestOutdatedVerticalTable1, "test", "0.0.1")
    args = {"_minor": 0, "input1": Input(type=DistDataType.OUTDATED_VERTICAL_TABLE)}
    comp: TestOutdatedVerticalTable1 = d1.make_component(args)
    assert comp.input1.type == DistDataType.VERTICAL_TABLE


def test_variable_input():
    class TestVariableInput(Component):
        input1: list[Input] = Field.input(
            "desc",
            types=[DistDataType.VERTICAL_TABLE],
            list_limit=Interval.closed(1, 2),
        )

        def evaluate(self) -> None: ...

    d1 = Definition(TestVariableInput, "test", "0.0.1", "test_variable_input")
    in0 = d1.component_def.inputs[0]
    assert in0.is_variable and in0.variable_min == 1 and in0.variable_max == 2
    param = NodeEvalParam(
        comp_id="test/test_variable_input:0.0.1",
        inputs=[Input(type=DistDataType.VERTICAL_TABLE)],
    )
    comp: TestVariableInput = d1.make_component(param)
    assert len(comp.input1) == 1
